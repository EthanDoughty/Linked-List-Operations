...
static struct Node* mergeLists(struct Node* list1, struct Node* list2)
{
    ...
    // Set the head to the smaller of the two list heads
    ...
    struct Node* current = head;  // Pointer to build the merged list
    ...
    // If one list is exhausted, link the remaining nodes
    if (list1 != NULL) {
        current->next = list1;
    } else {
        current->next = list2;
